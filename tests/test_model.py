...
class TestBombSiteTransformer:
    def _model(self, **kwargs) -> BombSiteTransformer:
        defaults = dict(input_dim=279, d_model=64, nhead=4,
    ...
    def test_output_shape(self):
        model = self._model()
        x = torch.randn(4, 240, 279)
        out = model(x)
        assert out.shape == (4, 3)

    def test_output_shape_short_sequence(self):
        model = self._model()
        x = torch.randn(2, 10, 279)
        out = model(x)
        assert out.shape == (2, 3)

    def test_no_nan(self):
        model = self._model()
        x = torch.randn(2, 50, 279)
        out = model(x)
        assert not torch.isnan(out).any()

    def test_batch_size_1(self):
        model = self._model()
        x = torch.randn(1, 240, 279)
        out = model(x)
