#!/usr/bin/env python3
"""
Portable HLTV Demo Pipeline — download, parse to parquet, delete .dem in one shot.

Usage (from USB or any machine):
    python run_pipeline.py
    python run_pipeline.py --config hltv_config.yaml
    python run_pipeline.py --dry-run

Each demo is:
  1. Downloaded & extracted to a temp dir
  2. Parsed to parquet -> data/processed/
  3. .dem deleted immediately (no disk accumulation)

Requires: pip install -r requirements_pipeline.txt
"""
...
from hltv.manifest import log_failure
...
def _seen_demos_from_parquets(processed_dir: Path) -> set[str]:
    """Return set of 'matchid_mapname' keys from existing parquets."""
    seen = set()
    for p in processed_dir.glob("*.parquet"):
        # e.g. "2391141_de_mirage.parquet" -> "2391141_de_mirage"
        seen.add(p.stem)
    return seen
...
def run(cfg: dict, dry_run: bool = False) -> None:
    ...
    seen_demos = _seen_demos_from_parquets(processed_dir)  # set of "matchid_mapname"
    ...
    existing_parquets = len(seen_demos)
    ...
    print(f"=== Portable HLTV Pipeline (download -> parse -> delete) ===")
    print(f"Existing parquets: {existing_parquets}  |  Target new parquets: {target}")
            ...
            with tempfile.TemporaryDirectory() as tmp:
                ...
                for dem_path in dem_paths:
                    ...
                    if demo_key in seen_demos:
                        print(f"    [skip] {demo_key}: already parsed")
                        continue

                    renamed = Path(tmp) / f"{demo_key}.dem"
                    ...
                    try:
                        ...
                        if result:
                            ...
                            print(f"    [skip] {demo_key}: no valid rounds")
                            skipped_count += 1
                    except Exception as e:
                        print(f"    [fail] {demo_key}: {str(e)[:100]}")
                        failed_count += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Portable HLTV pipeline: download -> parse -> cleanup")
    parser.add_argument(
        "--config",
